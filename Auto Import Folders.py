...
def sync_and_color_directory(media_pool, media_storage, current_bin, current_os_path):
    """Recursively mirrors OS folders into Resolve bins and colors matching clips."""
    ...
    if files_to_import:
        ...
        if imported_clips:
            for clip in imported_clips:
                ...
                if clip_path:
                    # Clean path to ignore dashes and underscores for foolproof matching
                    clean_path = clip_path.lower().replace("-", "").replace("_", "")
                    for keyword, color in config["CAMERA_KEYWORDS"].items():
                        ...
                        if clean_keyword in clean_path:
                            ...
                            break
            print(f" ---> Imported & Colored {len(imported_clips)} files into bin: '{current_bin.GetName()}'")
    ...
    for folder_name, folder_path in folders_to_scan:
        ...
        sync_and_color_directory(media_pool, media_storage, child_bin, folder_path)
...
def main():
    ...
    print("--- STARTING FOLDER SYNC & COLORING ---")
    for folder_path in config["FOLDERS_TO_SYNC"]:
        if os.path.exists(folder_path):
            ...
            sync_and_color_directory(media_pool, media_storage, target_bin, folder_path)
        else:
